...
// Прокси со временем протухают (у провайдера кончается срок аренды), поэтому
// берём с запасом: проверка мёртвого адреса занимает секунды, а заявка менеджера
// не должна падать из-за пары просроченных адресов подряд.
const MAX_PROXY_TRIES = Number(process.env.MAX_PROXY_TRIES) || 8
