...
export default function DashboardPage() {
  ...
  // ── Состояние ссылки ИП (вкладка временно скрыта, но код оставлен) ──
  ...
  // ── Состояние Счёта ИП (РКО, product_id=533) ──
  ...
  const [copiedAccountLink, setCopiedAccountLink] = useState(null)

  // Предупреждения за текущий месяц (баннер в шапке)
  const [warningMonthCount, setWarningMonthCount] = useState(0)
  ...
  const loadAccountHistory = useCallback(async () => {
    ...
    try {
      ...
      const data = await res.json()
      if (res.ok) setAccountHistory(data.applications || [])
    } catch (err) {
  ...
  useEffect(() => {
    if (activeTab === 'account-link' && user) loadAccountHistory()
  }, [activeTab, user, loadAccountHistory])

  // ИНН: 10 (ООО) или 12 (ИП) цифр
  const validateAccountINN = (inn) => {
  ...
  // ── Создание заявки Счёт ИП ──
  const handleCreateAccountLink = async (e) => {
    e.preventDefault()
    if (!validateAccountINN(accountForm.inn)) { setAccountError('Некорректный ИНН (10 для ООО, 12 для ИП)'); return }
    setAccountSubmitting(true); setAccountError(null); setAccountResult(null)
    try {
      const res = await authFetch('/api/account-link', {
        method: 'POST',
      ...
      const data = await res.json()
      if (!res.ok) { setAccountError(data.error || 'Ошибка') }
      else { setAccountResult(data); await loadAccountHistory() }
    } catch (err) {
      console.error('Account link create failed:', err)
      setAccountError(err.code === 'NO_SESSION' ? 'Сессия истекла, войдите заново' : 'Ошибка сети')
    } finally {
      setAccountSubmitting(false)
    }
  }

  // ── Копирование ссылки Счёт ИП ──
  const handleCopyAccountLink = async (link, id) => {
    ...
    setCopiedAccountLink(id)
    setTimeout(() => setCopiedAccountLink(null), 2000)
  }
  ...
  const handleSubmit = async (e) => {
    ...
    // 06.2026: упростили до 2 полей — Написавшие и Заказали РКО.
    // people_wrote = "Написавшие"; ordered_ip = "Заказали РКО".
    // Старые колонки (unsubscribed/replied/ordered_cards) в БД остаются,
    // но новые записи в них не пишутся — будут NULL.
    const record = {
      manager_id: user.id,
      date: form.date,
  ...
  // Тип команды сохранён для обратной совместимости, но в UI не разделяет.
  const weeklyValue = getLast7Days(reports, 'ordered_ip')
  const zone      = getZone(weeklyValue, 'standard')
  ...
  // Новый алгоритм вместо «Счёт ИП» — список команд и отдельных людей
  // в lib/bank-link-access.js
