...
// Заказали Симка (ordered_simka) за последние 7 дней — доп-продукт, вне конверсии.
function getSimkaLast7Days(reports) {
  const now = new Date()
  now.setHours(23, 59, 59, 999)
  const start = new Date(now); start.setDate(start.getDate() - 7); start.setHours(0, 0, 0, 0)
  return reports
...
const ZONE = {
  red:    { card: 'border-red-200 bg-red-50',    badge: 'bg-red-100 text-red-700 border border-red-200',    text: 'text-red-600',    label: 'Красная зона' },
  yellow: { card: 'border-amber-300 bg-amber-50', badge: 'bg-amber-100 text-amber-700 border border-amber-300', text: 'text-amber-600', label: 'Жёлтая зона' },
  green:  { card: 'border-emerald-200 bg-emerald-50',  badge: 'bg-emerald-100 text-emerald-700 border border-emerald-200',  text: 'text-emerald-600',  label: 'Зелёная зона' },
}
...
function getPersonalZone(ip, teamType) {
  ...
  const map = {
    red:    { bg: 'bg-red-50',    border: 'border-red-300',    text: 'text-red-600',    badge: 'bg-red-100 text-red-700 border border-red-300',    label: 'Красная зона' },
    yellow: { bg: 'bg-amber-50', border: 'border-amber-400', text: 'text-amber-600', badge: 'bg-amber-100 text-amber-700 border border-amber-400', label: 'Жёлтая зона' },
    green:  { bg: 'bg-emerald-50',  border: 'border-emerald-300',  text: 'text-emerald-600',  badge: 'bg-emerald-100 text-emerald-700 border border-emerald-300',  label: 'Зелёная зона' },
  }
  return map[key]
}
...
export default function TeamleadPage() {
  ...
  if (loading) {
    return <div className="min-h-screen bg-slate-50 flex items-center justify-center text-gray-600">Загрузка...</div>
  }
    ...
    <div className="min-h-screen bg-slate-50 text-gray-900">
          ...
          <div className="flex items-center justify-between sm:justify-start gap-3 sm:gap-8">
            ...
            <div className="flex items-center gap-2 sm:hidden">
              <div className="relative" ref={bellRef}>
                ...
                {showBell && (
                  <>
                    ...
                    <div style={{ backgroundColor: '#ffffff', border: '1px solid #cbd5e1' }}
                    ...
                    >
                      <div className="flex items-center justify-between mb-3">
                        ...
                        <button onClick={() => setShowBell(false)} className="text-gray-500 hover:text-gray-900 transition"><CloseIcon /></button>
                      </div>
                      {totalNotifications === 0 ? (
                        ...
                        <div className="space-y-2 max-h-80 overflow-y-auto">
                          ...
                          {redManagers.map(m => (
                            ...
                            >
                              <p className="text-red-700 text-sm font-semibold">{m.name}</p>
              ...
              {user && ADMIN_EMAILS.includes(user.email) && (
                <button onClick={() => router.push('/admin')} className="text-gray-600 hover:text-gray-900 text-xs transition">Админ</button>
              )}
              <button onClick={handleLogout} className="text-gray-500 hover:text-gray-900 text-sm transition">Выйти</button>
            </div>
          </div>
            ...
            {TABS.map(tab => (
              <button
                ...
                className={`px-3 sm:px-4 py-1.5 sm:py-2 rounded-lg text-sm font-medium transition-all whitespace-nowrap ${
                  activeTab === tab.id ? 'bg-teal-600 text-white' : 'text-gray-600 hover:text-gray-900 hover:bg-slate-100'
                }`}
              >
                {tab.label}
              </button>
          ...
          <div className="hidden sm:flex items-center gap-3">
            ...
            {user && ADMIN_EMAILS.includes(user.email) && (
              <button
                ...
                className="text-gray-600 hover:text-gray-900 text-sm transition px-3 py-1.5 rounded-lg hover:bg-slate-100"
              >
                Админ
              </button>
            )}
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 py-6 sm:py-8">
          ...
          <div className="space-y-10">
            ...
            <section>
              <div className="flex items-center gap-3 mb-4">
                ...
                <span className="text-gray-400 text-sm">тимлид</span>
              </div>

              {/* Zone strip */}
              <div className={`${myZone.bg} border ${myZone.border} rounded-2xl p-4 mb-4 flex items-center justify-between`}>
                <div>
                  <p className="text-gray-600 text-xs mb-1">Мои результаты за последние 7 дней</p>
              ...
              {/* Report form */}
              {showReportForm && (
                <div style={{ backgroundColor: '#ffffff', border: '1px solid #e2e8f0' }} className="rounded-2xl p-5 mb-4">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div>
                        ...
                        <input type="date" value={reportForm.date} onChange={e => setReportForm({ ...reportForm, date: e.target.value })}
                          className="w-full bg-white text-gray-900 px-4 py-2.5 rounded-lg border border-gray-300 focus:outline-none focus:border-teal-500 text-sm" required />
                      </div>
                      <div>
                        ...
                        <input type="number" min="0" value={reportForm.people_wrote} onChange={e => setReportForm({ ...reportForm, people_wrote: e.target.value })}
                          placeholder="0" className="w-full bg-white text-gray-900 px-4 py-2.5 rounded-lg border border-gray-300 focus:outline-none focus:border-teal-500 text-sm" />
                      </div>
                      <div>
                      ...
                      <div>
                        ...
                        <input type="number" min="0" value={reportForm.ordered_simka} onChange={e => setReportForm({ ...reportForm, ordered_simka: e.target.value })}
                          placeholder="0" className="w-full bg-white text-gray-900 px-4 py-2.5 rounded-lg border border-gray-300 focus:outline-none focus:border-teal-500 text-sm" />
                      </div>
                    </div>
                    <div className="flex gap-3 mt-4">
                      <button type="submit" disabled={submitting}
                        className="bg-teal-600 text-white hover:bg-teal-700 disabled:opacity-50 px-5 py-2 rounded-lg text-sm font-semibold transition">
                        {submitting ? 'Сохраняем...' : 'Сохранить'}
                      </button>
                      <button type="button" onClick={() => setShowReportForm(false)}
                        className="bg-slate-100 hover:bg-slate-200 px-5 py-2 rounded-lg text-sm transition">Отмена</button>
                    </div>
                  </form>
                </div>
              )}
              ...
              <div style={{ backgroundColor: '#ffffff', border: '1px solid #e2e8f0' }} className="rounded-2xl overflow-hidden overflow-x-auto">
                <table className="w-full min-w-[480px]" data-show-all={showAllMyReports}>
                  ...
                  <tbody>
                    {myReports.length === 0 ? (
                      <tr><td colSpan={5} className="text-center py-12 text-gray-400 text-sm">Нет данных — добавьте первый отчёт</td></tr>
                    ) : (showAllMyReports ? myReports : myReports.slice(0, 5)).map(r => (
                      <tr key={r.id} style={{ borderTop: '1px solid #f1f5f9' }} className="hover:bg-slate-50 group">
                        <td className="px-3 sm:px-5 py-3 text-sm text-gray-700">{new Date(r.date).toLocaleDateString('ru-RU')}</td>
                        ...
                        <td className="pr-4 py-3 text-right">
                          <button
                            ...
                            className="opacity-0 group-hover:opacity-100 text-gray-400 hover:text-red-600 transition disabled:opacity-30"
                            title="Удалить отчёт"
                          >
                            <TrashIcon />
                          </button>
              ...
              {myReports.length > 5 && (
                <div className="flex justify-center mt-3">
                  <button
                    ...
                    className="text-blue-600 hover:text-blue-700 text-sm font-medium px-4 py-2 transition"
                  >
                    {showAllMyReports
            ...
            <section>
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-gray-400 text-sm">{managers.length} менеджеров</span>
                </div>
                <button
                  onClick={() => { setShowAddModal(true); setAddError('') }}
                  className="bg-slate-100 hover:bg-slate-200 border border-slate-200 hover:border-slate-300 px-4 py-2 rounded-lg text-sm font-medium transition"
                >
                  + Добавить менеджера
                </button>
              </div>
              ...
              {managers.length === 0 ? (
                <div style={{ backgroundColor: '#ffffff', border: '1px solid #e2e8f0' }} className="rounded-xl p-8 text-center text-gray-400 text-sm">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
                  {managers.map(manager => {
                    ...
                    const simka7  = getSimkaLast7Days(mRep)
                      ...
                      >
                        <div className="flex justify-between items-start mb-3">
                          <div className="flex items-center gap-1.5 min-w-0">
                            ...
                            <span className="font-medium text-gray-900 text-sm leading-tight truncate">{manager.name}</span>
                          </div>
                          <div className="flex items-center gap-1 ml-1">
                            {alert14 && <span className="text-red-600 text-sm" title="14 дней в красной зоне">⚠</span>}
                            {!isDeletePending && (
                              <WarningButton
                        ...
                        {!isDeletePending ? (
                          <>
                            <div className="mb-3 space-y-0.5">
                              ...
                              <div>
                                ...
                                <span className="text-cyan-700 text-xs font-medium">{convStr(value7, wrote7)}</span>
                              </div>
                              <div>
                                <span className="text-gray-500 text-xs">Симка / 7 дн: </span>
                                <span className="text-amber-600 text-xs font-medium">{simka7}</span>
                              </div>
                            </div>
                          ...
                          <div onClick={e => e.stopPropagation()}>
                            <p className="text-gray-600 text-xs mb-3">Удалить менеджера?</p>
        ...
        {activeTab === 'daily' && (() => {
          ...
          const rows = teamMembers.map(member => {
            ...
            const report = memberReports.length > 0 ? {
              ...
              ordered_simka: memberReports.reduce((s, r) => s + (r.ordered_simka || 0), 0),
            ...
            <div className="space-y-6">
              ...
              <div style={{ backgroundColor: '#ffffff', border: '1px solid #e2e8f0' }} className="rounded-2xl p-5">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
                  <div>
                    ...
                    <p className="text-xl font-bold text-gray-900">{totals.people_wrote}</p>
                  </div>
                  <div>
                    <p className="text-gray-500 text-xs mb-1">Заказали РКО</p>
                    <p className="text-xl font-bold text-blue-600">{totals.ordered_ip}</p>
                  </div>
                  <div>
                    <p className="text-gray-500 text-xs mb-1">Конверсия</p>
                    <p className="text-xl font-bold text-cyan-700">{convStr(totals.ordered_ip, totals.people_wrote)}</p>
                  </div>
                  <div>
                    <p className="text-gray-500 text-xs mb-1">Заказали Симка</p>
                    <p className="text-xl font-bold text-amber-600">{totals.ordered_simka}</p>
                  </div>
                  <div>
              ...
              <section>
                <div className="flex items-center gap-3 mb-3">
                  <h2 className="text-base font-semibold text-gray-900">Команда {teamInfo?.name ?? profile?.team}</h2>
                      ...
                      {rows.map(({ member, report }) => {
                        ...
                        return (
                          <tr key={member.id} style={{ borderTop: '1px solid #f1f5f9' }} className="hover:bg-slate-50 transition">
                            ...
                            <td className="px-3 sm:px-5 py-3 text-sm text-cyan-700 font-medium">{report ? convStr(report.ordered_ip, report.people_wrote) : '—'}</td>
                            ...
                            {/* Скрыто 07.2026 (код сохранён): <td><CdValue value={sd?.debit} color="text-purple-600" /></td> */}
                          </tr>
                        )
                      })}
        ...
        {activeTab === 'salary' && (
          <div className="flex flex-col items-center justify-center py-32 text-center">
            ...
            <p className="text-gray-700 font-medium text-lg">Расчёт заработной платы</p>
          ...
          return (
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <h2 className="text-base font-semibold text-gray-900">Аккаунты команды</h2>
              ...
              {tgLoading && tgAccounts.length === 0 ? (
                ...
                <div className="text-center py-16 text-gray-400">Нет аккаунтов</div>
              ) : (
                <div style={{ backgroundColor: '#ffffff', border: '1px solid #e2e8f0' }} className="rounded-2xl overflow-hidden overflow-x-auto">
                            ...
                            {tgAssignSelect[acc.rowIndex] !== undefined ? (
                              <div className="flex items-center gap-1">
                                ...
                                <button onClick={() => setTgAssignSelect(prev => { const n = { ...prev }; delete n[acc.rowIndex]; return n })}
                                  className="text-gray-400 hover:text-gray-600 text-xs transition">Отмена</button>
                              </div>
                            ) : (
                            ...
                            <div className="flex gap-2">
                              ...
                              {acc.assignedTo && (
                                <button onClick={() => assignAccount(acc.rowIndex, '')}
                                  disabled={tgAssigning[acc.rowIndex]}
                                  className="text-red-500/80 hover:text-red-600 text-xs transition">Освободить</button>
                              )}
                            </div>
                          </td>
                        </tr>
                  ...
                  {distributedContacts.map((group, idx) => (
                    ...
                    >
                      <div className="flex justify-between items-center mb-3">
                        <span className="text-sm font-semibold text-gray-700">Аккаунт {idx + 1}</span>
                        <button
            ...
            <div>
              ...
              >
                {contactsLoading ? (
                  ...
                  <div className="text-center py-12 text-gray-400 text-sm">Вы ещё не получали контакты</div>
                ) : (
      ...
      {selectedManager && (
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4"
          ...
          >
            <div style={{ borderBottom: '1px solid #e2e8f0' }} className="px-6 py-5 flex justify-between items-start">
              <div>
              ...
              <button onClick={() => setSelectedManager(null)} className="text-gray-500 hover:text-gray-900 p-1 rounded-lg hover:bg-slate-100 transition"><CloseIcon /></button>
            </div>
              ...
              <div className="bg-slate-100/70 border border-gray-200 rounded-xl p-3 space-y-2">
                <div className="flex items-center justify-between">
                  <div className="min-w-0">
                    ...
                    <p className="text-gray-700 text-sm truncate">{selectedManager.email}</p>
                  </div>
                  <button
                    onClick={() => { navigator.clipboard.writeText(selectedManager.email); }}
                    className="text-gray-400 hover:text-gray-900 text-xs px-2.5 py-1 rounded-lg hover:bg-slate-100 transition flex-shrink-0 ml-2"
                  >
                    Копировать
                  </button>
                </div>
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-gray-700 text-sm">Arb2024!</p>
                  </div>
                  <button
              ...
              <table className="w-full">
                ...
                <tbody>
                  {modalReports.length === 0 ? (
                    <tr><td colSpan={5} className="text-center py-10 text-gray-400 text-sm">Нет отчётов</td></tr>
                  ) : modalReports.map(r => (
                    <tr key={r.id} style={{ borderTop: '1px solid #f1f5f9' }} className="hover:bg-slate-50 group">
                      <td className="py-2.5 text-sm text-gray-700">{new Date(r.date).toLocaleDateString('ru-RU')}</td>
            ...
            <div style={{ borderTop: '1px solid #e2e8f0' }} className="px-6 py-4 flex justify-end">
              {deleteConfirm === selectedManager.id ? (
                <div className="flex items-center gap-3">
                  ...
                  <button
                    onClick={() => setDeleteConfirm(null)}
                    className="bg-slate-100 hover:bg-slate-200 px-4 py-2 rounded-lg text-sm transition"
                  >
                    Отмена
                  </button>
                </div>
      ...
      {showAddModal && (
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4"
          ...
          <div style={{ backgroundColor: '#ffffff', border: '1px solid #cbd5e1' }}
            ...
            onClick={e => e.stopPropagation()}
          >
            <div style={{ borderBottom: '1px solid #e2e8f0' }} className="px-6 py-5 flex justify-between items-center">
              <div>
              ...
              <button onClick={() => setShowAddModal(false)} className="text-gray-500 hover:text-gray-900 p-1 rounded-lg hover:bg-slate-100 transition"><CloseIcon /></button>
            </div>
            ...
            <form onSubmit={handleCreateManager} className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-gray-600 text-xs mb-1.5 block">Имя</label>
                  <input
                ...
                <div>
                  <label className="text-gray-600 text-xs mb-1.5 block">Фамилия</label>
                  <input
              ...
              <div>
                <label className="text-gray-600 text-xs mb-1.5 block">
                  ...
                  {!addForm.emailManual && addForm.email && (
                    <span className="text-gray-400 ml-2">· сгенерирован автоматически</span>
                  )}
                </label>
                <input
                  ...
                  className={`w-full bg-white text-gray-900 px-4 py-2.5 rounded-lg border focus:outline-none focus:border-teal-500 text-sm ${
                    !addForm.emailManual && addForm.email ? 'border-blue-200' : 'border-gray-300'
                  }`}
                />
              </div>

              <div>
                <label className="text-gray-600 text-xs mb-1.5 block">Пароль</label>
                <input
              ...
              <div className="flex gap-3 pt-1">
                <button type="submit" disabled={addLoading}
                  className="flex-1 bg-teal-600 text-white hover:bg-teal-700 disabled:opacity-50 py-2.5 rounded-lg text-sm font-semibold transition">
                  {addLoading ? 'Создаём...' : 'Создать менеджера'}
                </button>
                <button type="button" onClick={() => setShowAddModal(false)}
                  className="bg-slate-100 hover:bg-slate-200 px-5 py-2.5 rounded-lg text-sm transition">Отмена</button>
              </div>
            </form>
          </div>
        </div>
