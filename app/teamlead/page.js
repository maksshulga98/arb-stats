...
const EMPTY_IP_FORM = { fullName: '', phone: '', email: '', city: '' }

function IpApplicationTab({ profile, scope }) {
    ...
    <>
      <div className="flex justify-between items-center mb-4">
        ...
        <div className="flex gap-2">
          {history.length > 0 && (
            <button onClick={downloadIpCSV} className="bg-gray-700 hover:bg-gray-600 px-4 py-2 rounded-lg text-sm font-medium transition">
              Скачать CSV
            </button>
          )}
          <button
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <input type="email" required value={form.email} onChange={e => setForm({ ...form, email: e.target.value })}
                    placeholder="client@example.com" className="w-full bg-gray-900 text-white px-4 py-2.5 rounded-lg border border-gray-700 focus:outline-none focus:border-blue-500 text-sm" />
                </div>
                <div>
                  ...
                  <input type="text" required value={form.city} onChange={e => setForm({ ...form, city: e.target.value })}
                    placeholder="Москва" className="w-full bg-gray-900 text-white px-4 py-2.5 rounded-lg border border-gray-700 focus:outline-none focus:border-blue-500 text-sm" />
                </div>
              </div>
          ...
          <table className="w-full min-w-[640px]">
            <thead>
              <tr style={{ borderBottom: '1px solid #1f1f2e' }}>
                ...
                {scope === 'all' && <th className="text-left px-4 py-3 text-gray-500 text-xs font-medium uppercase tracking-wider">Менеджер</th>}
                {scope === 'all' && <th className="text-left px-4 py-3 text-gray-500 text-xs font-medium uppercase tracking-wider">Команда</th>}
            ...
            <tbody>
              {history.map(app => (
                <tr key={app.id} style={{ borderTop: '1px solid #1a1a28' }} className="hover:bg-white/[0.02] transition">
                  ...
                  {scope === 'all' && <td className="px-4 py-3 text-sm text-gray-300">{app.manager_name || '—'}</td>}
                  {scope === 'all' && <td className="px-4 py-3 text-sm text-gray-500">{app.team || '—'}</td>}
