...
const STATUS_LABEL = {
  queued:     { text: 'В очереди',  cls: 'text-amber-600' },
  processing: { text: 'В работе…',  cls: 'text-blue-600' },
  success:    { text: 'Готово',     cls: 'text-emerald-600' },
  ...
  return (
    <>
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-base font-semibold text-gray-900">Заявка в банк</h2>
        <button
      ...
      <div style={{ backgroundColor: '#ffffff', border: '1px solid #e2e8f0' }} className="rounded-2xl overflow-hidden overflow-x-auto">
        {historyLoading ? (
          ...
          <div className="text-center py-12 text-gray-400 text-sm">Нет заявок — оформите первую</div>
        ) : (
          <table className="w-full min-w-[760px]">
            ...
            <tbody>
              {history.map(job => {
                ...
                return (
                  <tr key={job.id} style={{ borderTop: '1px solid #f1f5f9' }} className="hover:bg-slate-50 transition">
                    ...
                    {showManagerColumn && (
                      <td className="px-3 sm:px-5 py-3 text-sm text-gray-700">{managerNameById[job.manager_id] || '—'}</td>
                    )}
                    <td className="px-3 sm:px-5 py-3 text-sm text-gray-700">{bankLabel(job.bank)}</td>
                    <td className="px-3 sm:px-5 py-3 text-sm text-gray-700">{job.organization_name}</td>
                    <td className="px-3 sm:px-5 py-3 text-sm text-gray-600 font-mono">{job.inn}</td>
      ...
      {/* Модалка */}
      {showModal && (
        <div className="fixed inset-0 bg-slate-900/40 z-50 flex items-center justify-center p-4" onClick={() => { if (!submitting) setShowModal(false) }}>
          <div style={{ backgroundColor: '#ffffff', border: '1px solid #e2e8f0' }} className="rounded-2xl w-full max-w-md p-6 max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="flex justify-between items-center mb-4">
              ...
              <button onClick={() => { if (!submitting) setShowModal(false) }} className="text-gray-500 hover:text-gray-900 text-lg">✕</button>
            </div>

            {result ? (
              <div>
                <div className="bg-emerald-50 border border-emerald-300 rounded-lg p-4 mb-4">
                  <p className="text-emerald-700 text-sm font-semibold mb-2">Ссылка получена!</p>
                  <div className="flex items-center gap-2">
                    <input readOnly value={result.link} className="flex-1 bg-white text-sm text-gray-700 px-3 py-2 rounded-lg border border-gray-300 truncate" />
                    <button
                      onClick={() => handleCopy(result.link, 'modal')}
                      className={`px-3 py-2 rounded-lg text-xs font-medium transition shrink-0 ${
                        copiedId === 'modal' ? 'bg-emerald-100 text-emerald-700' : 'bg-teal-600 hover:bg-teal-700 text-white'
                      }`}
                    >
                ...
                <button onClick={() => { setShowModal(false); setResult(null) }} className="w-full bg-slate-100 hover:bg-slate-200 px-4 py-2.5 rounded-lg text-sm transition">
                  Закрыть
                </button>
              </div>
            ) : (
              <form onSubmit={handleCreate}>
                <div className="space-y-3">
                  <div>
                    <label className="text-gray-600 text-xs mb-1.5 block">Банк</label>
                    ...
                    {banks.length === 0 && <p className="text-amber-600 text-xs mt-1">Банки не настроены (нет ссылок оформления в env).</p>}
                  </div>
                  <div>
                ...
                {submitting && <p className="text-blue-700 text-sm mt-3 flex items-center gap-2"><span className="inline-block w-3 h-3 border-2 border-blue-500 border-t-transparent rounded-full animate-spin" />{statusText}</p>}
