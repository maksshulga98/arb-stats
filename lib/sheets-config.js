...
export const MANAGER_SHEETS = {
  ...
  // Команда Оли (Ari M перенесена сюда 16.05.2026 после расформирования команды
  // Ясмин; Маша Игнатова и Катя Шестакова тоже у Оли, но без личного листа)
  ...
  // Бывшие сотрудники команды Ясмин (расформирована 16.05.2026). В profiles
  // деактивированы (team=null), листы оставлены ради исторических отчётов.
...
export const CONTACTS_SPREADSHEET_ID = '1fMdlR5H1ZSlUfvs_8_NUdw0LPVOEnJ7AJdf6prtvBK8'

// Команда → индекс колонки (0 = A, 1 = B)
// 11.06.2026: после расформирования команды Карины — только Оля → колонка B
// (Ведение соц сетей). Колонка A "Копирайтер" сейчас никем не запрашивается.
...
// idx1 (столбец D) с 07.2026 — доп-продукт «Симка Билайн» (был ВТБ).
// В extractRow он идёт в отдельный bucket simka (не в debit, не в ip).
...
// A=0 (Всего ЦД), B=1 (Дата), C=2 (Альфа ИП), D=3..I=8 (дебетовые карты)
export const COL_TOTAL = 0
export const COL_DATE = 1
export const COL_PRODUCTS_START = 2
export const COL_PRODUCTS_END = 8  // inclusive (Альфа ИП + 6 дебетовых карт)

// ─── Таблица TG-аккаунтов ───
export const TG_ACCOUNTS_SPREADSHEET_ID = '1x180QOwAmjVAalfQPtH7CH5RlOeCa9qlt8FWn0S7ZdM'
