...
function extractRow(dayRow) {
  const products = {}
  let total = 0, ip = 0, debit = 0, simka = 0
  for (let col = COL_PRODUCTS_START; col <= COL_PRODUCTS_END; col++) {
    ...
    total += val
    // idx0 (Альфа ИП) → ЦД ИП; idx1 (столбец D, Симка Билайн) → отдельный
    // bucket simka (доп-продукт, вне конверсии); остальное → дебетовые.
    if (idx === 0) ip += val
    else if (idx === 1) simka += val
    else debit += val
  }
  return { total, ip, debit, simka, products }
}
...
export async function fetchSheetsData(names, dateFrom, dateTo) {
  ...
  await Promise.all(names.map(async (name) => {
    ...
    try {
      const acc = { total: 0, ip: 0, debit: 0, simka: 0, products: {} }
