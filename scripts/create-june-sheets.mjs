// Создаёт лист "Июнь" во всех таблицах активных менеджеров.
// Эталон — лист "Май" в таблице Карины Калининой (она шаблон-носитель формата).
// Копируем через Sheets API copyTo (форматы, цвета, формулы сохраняются), потом:
//   - переименовываем "Копия Май" → "Июнь"
//   - перемещаем на 1-ю позицию
//   - чистим C4:I36
//   - проставляем даты 01.06 … 30.06 в B4:B33
//   - если "Июнь" уже есть — удаляем и создаём заново
//
// Запуск:
//   node --env-file=.env.local scripts/create-june-sheets.mjs --dry
//   node --env-file=.env.local scripts/create-june-sheets.mjs --manager "Сабрина  Садыкова"
//   node --env-file=.env.local scripts/create-june-sheets.mjs --all
...
const TEMPLATE_TAB_NAME = 'Май'
const NEW_TAB_NAME = 'Июнь'
const NEW_MONTH_NUM = 6
const DAYS_IN_MONTH = 30 // в июне 30 дней
...
async function migrateManager(sheets, name, spreadsheetId) {
  ...
  log(`переименовываю → "${NEW_TAB_NAME}", index=0, чищу C4:I36, ставлю даты 01.06–30.06`)
  if (!isDry) {
    const requests = [
      {
      ...
      // 31-я строка (B34) — её быть не должно, очищаем на случай если в шаблоне Май там стояло "31.05"
      {
        updateCells: {
          range: {
            sheetId: newSheetId,
