// Создание вкладок «Август» всем активным членам команд.
//
// Отличие от прошлых месяцев: эталон НЕ из таблицы Карины (там нет Июля и
// формат старый), а ИЮЛЬСКАЯ ВКЛАДКА САМОГО МЕНЕДЖЕРА — так новый формат
...
//   1. находит свою вкладку «Июль»
//   2. если «Август» уже есть — ПЕРЕИМЕНОВЫВАЕТ в «Август <год-1>» (не удаляет!),
//      чтобы не потерять исторические данные прошлого сезона
//   3. дублирует «Июль» → «Август», ставит первой вкладкой
//   4. чистит данные C4:I36 и проставляет даты 01.08–31.08
//
// Запуск:  node scripts/create-august-sheets.mjs          (DRY-RUN, только план)
//          APPLY=1 node scripts/create-august-sheets.mjs   (реальное создание)
...
const NEW_TAB_NAME = 'Август'
const SRC_TAB_NAME = 'Июль'
const NEW_MONTH_NUM = 8
const DAYS_IN_MONTH = 31
const ARCHIVE_SUFFIX = ' 2025'   // старая августовская вкладка → «Август 2025»
...
async function processManager(p) {
  ...
  // Старую вкладку «Август» (прошлогоднюю) не удаляем — архивируем
  ...
  log(`дублирую «${SRC_TAB_NAME}» → «${NEW_TAB_NAME}», чищу C4:I36, даты 01.08–${DAYS_IN_MONTH}.08`)
  if (!APPLY) return true

  // Дублируем июльскую вкладку внутри той же таблицы
